...
static void uinput_listen()
{
	...
	std::map<int, rumble_effect> effectMap;
	...
	while (!io_canceled()) {
		...
		case EV_UINPUT:
			...
			case UI_FF_UPLOAD:
				...
				if (!old_rumble_mode) {
					if(effectMap.find(upload.effect.id) == effectMap.cend()) {
						effectMap[upload.effect.id] = rumble_effect(
							(int)upload.effect.id,
							(int)weak,
							(int)strong,
							(int)timeout
						);
						if (debug) syslog(LOG_INFO, "going for effect #%i", upload.effect.id);
					}
				} else {
					active = true;
				}
			...
			case UI_FF_ERASE:
				...
				if (!old_rumble_mode) {
					if(effectMap.find(event.value) != effectMap.cend()) {
						effectMap.erase(event.value);
					}
				} else {
					weak = 0;
					strong = 0;
		...
		case EV_FF:
			if (event.value) {
				if(effectMap.find(event.code) != effectMap.cend()) {
					do_rumble(csk, led_n, effectMap[event.code].weak, 
						effectMap[event.code].strong, effectMap[event.code].timeout);
					if (debug) syslog(LOG_INFO, "RUMBLE now :: %i|%i|%i", effectMap[event.code].weak, 
						effectMap[event.code].strong, effectMap[event.code].timeout);
				}
			} else {
				if (!old_rumble_mode) {
